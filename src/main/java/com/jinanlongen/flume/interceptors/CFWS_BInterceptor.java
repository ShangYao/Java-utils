...
/**  
*   an interceptor for flume to deal with  file from CFWS_B,
...
public class CFWS_BInterceptor implements Interceptor {
	private final Charset charset;

	private CFWS_BInterceptor(Charset charset) {
	...
	public void erro(String contex, String pref) {
		String filename = GetProp.getARTDBprops().getString("errofilefold") + "cfwsb-" + pref + ".txt";
	...
	public void errofile(String basename, String origBody, String pref) {
		String filename = GetProp.getARTDBprops().getString("errofilefold") + "cfwsb-" + pref + basename;
		File file = new File(filename);
		FileOutputStream out = null;
		try {
			out = new FileOutputStream(file, true);
	...
	public static class Builder implements org.apache.flume.interceptor.Interceptor.Builder {
		...
		public Interceptor build() {
			return new CFWS_BInterceptor(this.charset);
		}
